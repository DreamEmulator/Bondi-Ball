...
vec3 doffset(float z, float time) {
  float eps = 0.05;
  return (offset(z + eps, time) - offset(z - eps, time))/(2.0*eps);
}

vec3 ddoffset(float z, float time) {
  float eps = 0.05;
  return (doffset(z + eps, time) - doffset(z - eps, time))/(2.0*eps);
}
...
vec3 color(vec3 ww, vec3 uu, vec3 vv, vec3 ro, vec2 p, vec2 size, float time) {
  float lp = length(p);
  vec2 np = p + 1.0/size;
  float rdd = 3.0;
...
vec3 effect(vec2 p, vec2 pp, float time, vec2 size) {
  float tm  = planeDist*time*BPM/60.0;
  vec3 ro   = offset(tm, time);
  vec3 dro  = doffset(tm, time);
  vec3 ddro = ddoffset(tm, time);

  vec3 ww = normalize(dro);
  vec3 uu = normalize(cross(normalize(vec3(0.0,1.0,0.0)+ddro), ww));
  vec3 vv = cross(ww, uu);

  vec3 col = color(ww, uu, vv, ro, p, size, time);
...
void main() {
  vec2 q = v_tex_coord*u_sprite_size/u_sprite_size;
  vec2 p = -1. + 2. * q;
  vec2 pp = p;
  p.x *= u_sprite_size.x/u_sprite_size.y;

  vec3 col = effect(p, pp, u_time, u_sprite_size);
